...
if sys.platform == 'win32':
    ...
    def find_coc_hwnd() -> int | str | None:
        result = {}
        def _cb(hwnd, ctx):
    ...
    def get_client_rect(hwnd: int | str) -> tuple[int, int, int, int]:
    ...
    def capture_frame(hwnd: int | str) -> np.ndarray | None:
        try:
    ...
    def click_relative_roi(hwnd: int | str, roi: dict):
        cx, cy, cw, ch = get_client_rect(hwnd)
    ...
    def get_relative_mouse_pos(hwnd: int | str) -> tuple[float, float]:
        try:
    ...
    def scroll_roi(hwnd: int | str, roi: dict):
        cx, cy, cw, ch = get_client_rect(hwnd)
        ch_capture = max(ch - HEADER_OFFSET_PX, 1)
    ...
    def focus_sea_background(hwnd: int | str):
        ZOOM_OUT_TICKS  = 15
        ZOOM_IN_TICKS   = 15
    ...
    def zoom_camera(hwnd: int | str, ticks: int, direction="in"):
        cx, cy, cw, ch = get_client_rect(hwnd)
        ch_capture = max(ch - HEADER_OFFSET_PX, 1)
    ...
    def _run_adb(cmd: str) -> bytes:
        global _adb_device
        target = f"-s {_adb_device}" if _adb_device else ""
        res = subprocess.run(f"adb {target} {cmd}", shell=True, capture_output=True)
        return res.stdout
        
    def find_coc_hwnd() -> str | None:
        global _adb_device, _adb_width, _adb_height
        
        # Connect to Waydroid ADB daemon automatically
        ip_res = subprocess.run("waydroid status | grep 'IP address:' | awk '{print $3}'", shell=True, capture_output=True, text=True)
        waydroid_ip = ip_res.stdout.strip()
        if waydroid_ip:
            subprocess.run(f"adb connect {waydroid_ip}:5555", shell=True, capture_output=True)
        else:
            subprocess.run("adb connect 192.168.240.112:5555", shell=True, capture_output=True)
        
        res = subprocess.run("adb devices", shell=True, capture_output=True, text=True)
        if "command not found" in res.stderr or "command not found" in res.stdout:
            print("[ERROR] adb is not installed. Please run: sudo pacman -S android-tools")
            return None
        ...
        if not devices:
            print("[ERROR] No ADB devices found. Ensure Waydroid is running and ADB is enabled.")
            return None
            
        _adb_device = devices[0]
        print(f"[OK] ADB Device found: {_adb_device}")
        
        # Get screen size
        res = subprocess.run(f"adb -s {_adb_device} shell wm size", shell=True, capture_output=True, text=True)
        try:
            size_str = res.stdout.strip().split(":")[-1].strip()
            w, h = map(int, size_str.split("x"))
            if w > h:
                _adb_width, _adb_height = w, h
            else:
                _adb_width, _adb_height = h, w  # Clash of clans is landscape
        except:
            pass
    ...
    def get_client_rect(hwnd: int | str) -> tuple[int, int, int, int]:
        return 0, 0, _adb_width, _adb_height

    def capture_frame(hwnd: int | str) -> np.ndarray | None:
        try:
            out = _run_adb("exec-out screencap -p")
            if not out: return None
            arr = np.frombuffer(out, np.uint8)
            img = cv2.imdecode(arr, cv2.IMREAD_COLOR)
            return img
        except Exception as e:
            print(f"[ERROR] adb capture_frame: {e}")
            return None
    ...
    def click_relative_roi(hwnd: int | str, roi: dict):
        ...
        screen_x = int(center_x * _adb_width)
        screen_y = int(center_y * _adb_height)
        
        _run_adb(f"shell input tap {screen_x} {screen_y}")

    def get_relative_mouse_pos(hwnd: int | str) -> tuple[float, float]:
        # Mouse coordinate UI is mostly for debugging, unsupported in ADB mode
        return (-1.0, -1.0)
    ...
    def scroll_roi(hwnd: int | str, roi: dict):
        ...
        end_y = screen_y - 300
        _run_adb(f"shell input swipe {screen_x} {screen_y} {screen_x} {end_y} 300")
        time.sleep(0.6)

    def focus_sea_background(hwnd: int | str):
        center_x = _adb_width // 2
        center_y = _adb_height // 2
        
        for _ in range(3):
            _run_adb(f"shell input swipe {center_x} {center_y} {center_x} {center_y - 400} 200")
        for _ in range(3):
            _run_adb(f"shell input swipe {center_x - 400} {center_y} {center_x + 400} {center_y} 200")
        time.sleep(0.5)
    ...
    def zoom_camera(hwnd: int | str, ticks: int, direction="in"):
        ...
        # Simple swipe for anti-afk purposes if actual pinch-to-zoom is unsupported via ADB shell input
        if direction == "in":
            _run_adb(f"shell input swipe {center_x} {center_y} {center_x} {center_y + 300} 200")
        else:
            _run_adb(f"shell input swipe {center_x} {center_y + 300} {center_x} {center_y} 200")
        time.sleep(0.2)

    def pan_camera(hwnd: int | str, start_rel: tuple, end_rel: tuple, drag_speed=10):
        start_x = int(_adb_width * start_rel[0])
        start_y = int(_adb_height * start_rel[1])
        end_x = int(_adb_width * end_rel[0])
        end_y = int(_adb_height * end_rel[1])
